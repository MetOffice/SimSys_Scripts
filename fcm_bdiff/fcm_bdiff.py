...
class FCMBase:
    ...
    def __init__(self, parent=None, repo=None):
        ...
        # Implies suite usage...
        self._branch, self._retries = use_mirror(repo or ".")
        self._branch_info = get_branch_info(self._branch, retries=self._retries)

        self._branch_url = get_url(self._branch_info)
        self.parent = parent or get_branch_parent(self._branch_info)
       ...
       # The branch should not be the trunk (a branch-diff would make no sense)
        if is_trunk(self._branch_url):
            print("{} appears to be the trunk, nothing to do!".format(self._branch_url))
        self.is_branch = not is_trunk(self._branch_url) # Historically this was the test
...
class FCMInfo(FCMBase):
    ...
    def repository_root(self) -> str:
        """Get the repository root from branch info."""
        match = re.search(
            r"^Repository Root:\s*(?P<url>.*)\s*$", self.branch_info, flags=re.MULTILINE
        )
        if match:
    ...
    @property
    def branch_parent(self) -> str:
        """Get the branch parent from branch info."""
        match = re.search(
            r"^Branch Parent:\s*(?P<parent>.*)$", self.branch_info, flags=re.MULTILINE
        )
        if match:
    ...
    @property
    def url(self) -> str:
        """Get the URL from branch info."""
        match = re.search(r"^URL:\s*(?P<url>.*)$", self.branch_info, flags=re.MULTILINE)
        if match:
            return match.group("url")
        else:
            raise Exception("Could not find URL field")