...
class FCMBase:
    ...
    # Not sure this will be required/used.
    # It's just the git version defines it.
    ...
    def __init__(self, parent=None, repo=None):
        ...
        # is the branch that was taken from the trunk (to avoid test
        # branches etc)
        ...
        # The branch parent(ancestor in git_bdiff) should be the trunk(main);
        # if it isn't assume this is a
        # branch-of-branch (a test branch), and redirect the request to point
    ...
    def get_branch_name(self):
        ...
        For now, define it to be the contants of the URL after .*/main/ has
        been stripped off. i.e. it will start with trunk/... or branches/...
        """
    ...
    def use_mirror(self, branch):
        ...
        If using the mirror then fcm calls can sometimes fail so specify a
        number of retries for other routines to use.
    ...
    def get_branch_parent(self):
        """
        Given the raw output from an fcm binfo command - which can be
        retrieved by calling get_branch_info() - returns the Branch Parent
        Field
        """
        ...
        if parent:
            ...
            # Will end up here if _branch is the trunk. In which case we shold
            #  possibly return _branch?
            parent = re.search(
                r"^URL:\s*(?P<parent>.*)$",
                self._branch_info,
                flags=re.MULTILINE,
    ...
    def get_url(self):
        ...
        url = re.search(r"^URL:\s*(?P<url>.*)$", self._branch_info,
                        flags=re.MULTILINE)
        if url:
            url = url.group("url")
        else:
    ...
    def get_repository_root(self):
        """
        Given the raw output from an fcm binfo command - which can be
        retrieved by calling get_branch_info() - returns the Repository Root
        field
        """
    ...
    def get_latest_commit(self):
        """
        Given the raw output from an fcm binfo command - which can be
        retrieved by calling get_branch_info() - returns the Last Changed Rev
        """
...
class FCMBDiff(FCMBase):
    ...
    def _get_files(self, path_override=None):
        ...
        # format if the branch has been reversed off the trunk. The expected
        # format is
        # svn://fcm1/um.xm_svn/main/trunk/rose-stem/bin/suite_report.py
        ...
        # Strip whitespace, and remove blank lines while turning the output
        ...
        # construct the base path of the trunk URL and compare it to the
        # results of the bdiff command above
        ...
        if path_override is not None:
            ...
            base = path_override
            bdiff_files = [os.path.join(base, bfile) for
                           bfile in relative_paths]
        elif base_source_key in os.environ:
            # If running as a suite, the base path to the working copy
            # can be used
            # However, unless the suite task is running on a machine with
            # the same path to the working copy, the task can't really make
            # much use of this.
